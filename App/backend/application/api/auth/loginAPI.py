...
from application.data.models import db,User,UserRoles,Role
...
class LoginAPI(Resource):
    def post(self):
        ...
        print(user.roles)
        if 4 in roles:
            return jsonify({'status': 'failed','message': 'User Blocked !!'})
        ...
        return jsonify({'status': 'success','message': 'Successfully logged in !!', 'access_token': access_token, 'refresh_token': refresh_token , "user_mail": user_mail, "admin" : user.admin,"roles":roles, "user_id":user.user_id})
